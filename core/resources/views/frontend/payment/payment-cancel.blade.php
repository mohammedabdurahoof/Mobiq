        ...
        <div class="payment-cancel-wrapper">
            <div class="payment-contents">
                ...
                <div class="btn-wrapper margin-top-40">
                    @if(auth('web')->check())
                    <a href="{{ route('user.home') }}" class="btn-default transparent-btn-2">{{ __('Go to Dashboard') }}</a>
                    @else
                    <a href="{{ route('homepage') }}" class="btn-default transparent-btn-2">{{ __('Back to Home') }}</a>
                    @endif
                </div>
            </div>
        </div>
