                    ...
                    <div class="col-lg-12">
                                    ...
                                    <tr>
                                        ...
                                        @foreach ($products as $product)
                                            <td>
                                                    ...
                                                    @if ($inventory)
                                                        @if ($inventory->sum('stock_count') - $inventory->sum('sold_count') > 0)
                                                            <span class="same-s-1 stock">{{ __('In Stock') }}</span>
                                                        @else
                                                           <span class="same-s-1 outofstock"> {{ __('Out of Stock') }}</span>
                                                        @endif
                                                    @else
                                                        <span class="same-s-1 outofstock">{{ __('Not Available') }}</span>
                                                    @endif
                                        </td>
                                        @endforeach
                                    </tr>
                                    ...
                                    @foreach ($products as $product)
                                        <td>
                                            @if ($product->attributes)
                                                <a href="{{ route('frontend.products.single', $product->slug) }}" class="default-btn browse-btn">{{ __('View Options') }}</a>
                                            @else
                                                <a href="#" data-attributes="{{ $product->attributes }}" data-id="{{ $product->id }}" class="default-btn add_to_cart_ajax browse-btn">{{ __('Buy Now') }}</a>
                                            @endif
                                        </td>
                                    @endforeach
                                </tr>
                        ...
                        <div class="btn-wrapper d-flex justify-content-between">
                            @if(isset(request()->index) && request()->index > 1)
                                <a href="{{ route('frontend.products.compare', ['index' => request()->index - 1]) }}" class="btn-default color-reverse">{{ __('Previous Item') }}</a>
                            @endif
                            @php
                                $next_index = isset(request()->index) && request()->index > 1 ? request()->index : 1;
                            @endphp
                            <a href="{{ route('frontend.products.compare', ['index' => $next_index + 1]) }}" class="btn-default color-reverse">{{ __('Next Item') }}</a>
                        </div>
                    </div>
                </div>
            </div>
