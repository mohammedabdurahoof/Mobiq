                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{route('admin.general.typography.settings')}}" method="POST" enctype="multipart/form-data">
                            ...
                            <h4 class="header-title margin-top-80">{{__("Extra Font Typography Settings")}}</h4>
                            <div class="form-group">
                                <label for="heading_font">{{__('Extra Font')}}</label>
                                <label class="switch">
                                    <input type="checkbox" name="extra_font"  @if(!empty(get_static_option('extra_font'))) checked @endif id="extra_font">
                                    <span class="slider"></span>
                                </label>
                                <small>{{__('Use different font family for heading tags ( h1,h2,h3,h4,h5,h6)')}}</small>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control nice-select wide" name="extra_font_family" id="extra_font_family">
                                    @foreach($google_fonts as $font_family => $font_variant)
                                        <option value="{{$font_family}}" @if($font_family == get_static_option('extra_font_family')) selected @endif>{{$font_family}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group margin-top-50">
