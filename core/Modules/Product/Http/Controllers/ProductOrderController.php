    ...
    public function orderLogs()
    {
       if(auth()->user()->role=="editor"){   
        $all_orders = ProductSellInfo::all();
            ...
            $all_orders = ProductSellInfo::where('created_by', auth()->user()->id)->paginate(10);
        }
        foreach($all_orders as $val){
            $db_order_details = json_decode($val->order_details, true);
            $db_order_details = is_string($db_order_details) ? json_decode($db_order_details, true) : $db_order_details;
            $products = Product::where('id', array_keys($db_order_details))->first();
            // dd(array_keys($db_order_details));
            $val['title']=$products->title ?? '--';
            $val['image']=$products->image ?? '--';
            $val['attribute']=$val->order_details;
        } 
