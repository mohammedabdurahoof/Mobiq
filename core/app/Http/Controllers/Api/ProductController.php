...
class ProductController extends Controller
{
    ...
    public function productDetail($id){
        ...
        $product->inventory->sold_count = (int) $product->inventory->sold_count ?? 0;
        $product->inventory->product_id = (int) $product->inventory->product_id ?? 0;
        $product->inventory->stock_count = (int) $product->inventory->stock_count ?? 0;

        $product->tags->transform(function ($item){
        ...
        // foreach($product->rating as $ratting){
        //     $prd_ratting[] = [
        //         "id" => (int) $ratting->id,
        //         "user_id" => (int) $ratting->user_id,
        //         "product_id" => (int) $ratting->product_id,
        //         "status" => (int) $ratting->status,
        //         "ratting" => (int) $ratting->ratting,
        //         "review_msg" => $ratting->review_msg,
        //         "created_at" => $ratting->created_at,
        //         "updated_at" => $ratting->updated_at,
        //     ];
        // }
        
        // $product->rating = $prd_ratting;
        ...
        $product->inventory_details_count = (int)$product->inventory_details_count;
        
        if(!empty($product->campaign)){
            $product->campaign->product_id = (int) $product->campaign->product_id;
            $product->campaign->campaign_id =(int) $product->campaign->campaign_id;
            $product->campaign->units_for_sale = (int) $product->campaign->units_for_sale;
        }
        
        if(!empty($product->campaignProduct)){
            $product->campaignProduct->product_id = (int) $product->campaignProduct->product_id;
            $product->campaignProduct->campaign_id = (int) $product->campaignProduct->campaign_id;
            $product->campaignProduct->units_for_sale = (int) $product->campaignProduct->units_for_sale;
        }
        
        $product->inventoryDetails?->transform(function ($item){
            $item->product_id = (int) $item->product_id;
            $item->inventory_id = (int) $item->inventory_id;
            $item->additional_price = (int) $item->additional_price;
            $item->stock_count = (int) $item->stock_count;
            $item->sold_count = (int) $item->sold_count;
            $item->includedAttributes->transform(function ($id_item){
                $id_item->product_id = (int)$id_item->product_id;
                $id_item->inventory_details_id = (int)$id_item->inventory_details_id;
                return $id_item;
            });
            return $item;
        });
        ...
        if(!empty($product->additionalInfo)){
            $product->additionalInfo->transform(function ($item){
                $item->product_id = (int) $item->product_id;
                return $item;
            });
        }
        ...
        $ratings = ProductRating::where('product_id', $product->id)->with('user')->get()->transform(function ($item){
            $item->product_id = (int) $item->product_id;
            $item->user_id = (int) $item->user_id;
            $item->status = (int) $item->status;
            $item->rating = (int) $item->rating;
            $item->user = \App\User::find($item->user_id);
        ...
        $maximum_available_price = (int) Product::query()->with('category')->max('price');
        $min_price = (int) request()->pr_min ? request()->pr_min : Product::query()->min('price');
        $max_price = (int) request()->pr_max ? request()->pr_max : $maximum_available_price;
    ...
    public function priceRange(){
        ...
        return response()->json(["min_price" => (int)$min_price, "max_price" => (int)$max_price]);
    }
