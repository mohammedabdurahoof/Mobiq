...
class OrderController extends Controller
{
    ...
    public function details(ProductSellInfo $ProductSellInfo){
        ...
        $ProductSellInfo->order_details = json_decode($ProductSellInfo->order_details);
        $ProductSellInfo->payment_meta = json_decode($ProductSellInfo->payment_meta);

        $product = \App\Product\Product::select(["id","title","image","price","sale_price","badge","slug","attributes"])->without("inventory")->whereIn("id",$ids)->get()
            ...
            $item->campaign_percentage = round($campaign_percentage,2);
        ...
        return response()->json(["order_info" => $ProductSellInfo,"shipping_address" => $user_shipping_address,"product" => $product,"country" => optional($country)->name ?? null,"state" => optional($state)->name]);
    }
}
