...

{
...

    {
        $user = auth()->user();
        if($user->role == 'editor'){
            $all_blogs_count = ProductSellInfo::where('created_by' , $user->id)->count();

            $all_products_count = Product::where('created_by' , $user->id)->count();

            $all_completed_sell_count = ProductSellInfo::where('user_id' , $user->id)->where('status', 'complete')->count();
    
            $all_pending_sell_count = ProductSellInfo::where('user_id' , $user->id)->where('status', 'pending')->count();
    
            $total_ongoing_campaign = Campaign::where('status', 'publish')->count();
    
            $total_sold_amount = ProductSellInfo::where('user_id' , $user->id)->where('status', 'complete')->sum('total_amount');
            ...
            $all_completed_sell_count = ProductSellInfo::where('status', 'complete')->where('created_by' , $user->id)->count();
    
            $all_pending_sell_count = ProductSellInfo::where('status', 'pending')->where('created_by' , $user->id)->count();
    
            $total_ongoing_campaign = Campaign::where('status', 'publish')->count();
    
            $total_sold_amount = ProductSellInfo::where('status', 'complete')->where('created_by' , $user->id)->sum('total_amount');
        ...
        return view('backend.admin-home')->with([
            ...
            'all_blogs_count' => $all_blogs_count ?? null,

            'all_products_count' => $all_products_count ?? null,

            'all_completed_sell_count' => $all_completed_sell_count ?? null,

            'total_ongoing_campaign' => $total_ongoing_campaign ?? null,

            'all_pending_sell_count' => $all_pending_sell_count ?? null,

            'total_sold_amount' => float_amount_with_currency_symbol($total_sold_amount) ?? null,



            'sell_per_month' => $sell_per_month ?? null,

            'user_enroll_per_month' => $user_enroll_per_month ?? null,
