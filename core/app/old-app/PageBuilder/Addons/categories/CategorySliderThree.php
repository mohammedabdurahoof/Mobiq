...
class CategorySliderThree extends PageBuilderBase
{
    use RepeaterHelper;
    /**
     * @inheritDoc
    ...
    public function frontend_render()
    {
        ...
        return $this->renderBlade("category.category_slider_three", compact(["padding_top", "padding_bottom", "categories","section_title"]));
    }
    ...
    private function prepareForFetchingData($categories){
        ...
        if(!empty($categories)){
            $data = ProductCategory::select("title","id","image")
                ->where('status', 'publish')->whereIn("id",$categories)->get();
        }else{
            $data = ProductCategory::select("title","id","image")
                ->withCount("product")->where('status', 'publish')
                ->take(10)->get();
        }
